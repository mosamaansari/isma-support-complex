...
export default function Reports() {
  ...
  if (!currentUser || !hasResourcePermission(currentUser.role, 'reports:view', currentUser.permissions)) {
    return (
      <div className="p-8 text-center">
        <p className="text-red-600 dark:text-red-400">
          Access denied. Insufficient permissions to view reports.
        </p>
      </div>
    );
  }
  ...
  if (isUsingBackendData && dateRangeReport) {
    ...
    const exportToPDF = async () => {
      ...
      reportsToProcess.forEach((report: any, reportIdx: number) => {
        ...
        if (allTransactions.length > 0) {
          ...
          allTransactions.forEach((tran) => {
            ...
            <td style="border: 1px solid #e5e7eb; padding: 8px; max-width: 200px; overflow: hidden; text-overflow: ellipsis;">${tran.description || "-"
              }</td>
            <td style="border: 1px solid #e5e7eb; padding: 8px; text-transform: capitalize;">${tran.paymentType === "cash"
                ? "Cash"
                : tran.paymentType === "card"
                  ? "Card"
                  : "Bank Transfer"
              }</td>
            <td style="border: 1px solid #e5e7eb; padding: 8px; font-size: 10px;">${tran.bankName || "-"
              }</td>
            <td style="border: 1px solid #e5e7eb; padding: 8px; text-align: right;">${tran.beforeBalance !== null && tran.beforeBalance !== undefined
            ...
            <td style="border: 1px solid #e5e7eb; padding: 8px; text-align: right;">${tran.afterBalance !== null && tran.afterBalance !== undefined
      ...
      <html>
        <head>
          <title>Report - ${dateRange.start}${dateRange.start !== dateRange.end ? ` to ${dateRange.end}` : ""
        }</title>
          <style>
            @media print {
              @page { margin: 15mm; }
        ...
        <body>
          ...
          <p><strong>Date Range:</strong> ${dateRange.start}${dateRange.start !== dateRange.end ? ` to ${dateRange.end}` : ""
        }</p>
          ${printContent}
          <script>
      ...
      <div>
        ...
        <div className="mb-6" ref={printRef}>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 mb-4 no-print">
            ...
            {currentUser && hasResourcePermission(currentUser.role, 'opening_balance:view', currentUser.permissions) && (
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:gap-4 mb-6 sm:grid-cols-2 md:grid-cols-4 no-print">
                ...
                <div className="p-3 sm:p-4 bg-white rounded-lg shadow-sm dark:bg-gray-800">
                  ...
                  <p
                    className={`text-lg sm:text-xl lg:text-2xl font-bold price-responsive ${(isUsingBackendData ? closingBalance : profit) >= 0
                        ...
                        (transaction: any, index: number) => {
                          ...
                          return (
                            <tr
                              ...
                              className={`border-b border-gray-100 dark:border-gray-700 ${index % 2 === 0
                                ? isIncome
                            ...
                            >
                              ...
                              <td
                                className={`p-2 text-right font-semibold whitespace-nowrap ${isIncome
                                  ? "text-green-600 dark:text-green-400"
                                  : "text-red-600 dark:text-red-400"
                                  }`}
                              >
            ...
            (dateRangeReport &&
              ...
              <div className="mb-6 bg-white rounded-lg shadow-sm dark:bg-gray-800">
                ...
                >
                  ...
                  <svg
                    className={`w-5 h-5 text-gray-600 dark:text-gray-400 transition-transform ${isDateWiseFlowExpanded ? "rotate-180" : ""
                      }`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                ...
                <div
                  className={`overflow-hidden transition-all duration-300 ease-in-out ${isDateWiseFlowExpanded ? "max-h-[600px]" : "max-h-0"
                    }`}
                >
                  <div
                    className={`p-3 sm:p-4 md:p-6 pt-0 space-y-4 sm:space-y-6 ${isDateWiseFlowExpanded
